...
class RekapPresensiService
{
    ...
    public function rekapByRentangWaktu($id_kelas, $tahun, $start_date, $end_date)
    {
        $res = DB::select('
        SELECT * FROM
            (SELECT G.*,S.nama,S.nisn FROM
            ...
            LEFT JOIN (
                ...
                GROUP BY id_grouping) AS P
            ON LS.id_grouping = P.id_grouping
        ', [$id_kelas, $tahun, $start_date, $end_date]);
        return $res;
    }

    public function rekapByKelasTahun($id_kelas, $tahun)
    {
        $res = DB::select('
        SELECT * FROM
            (SELECT G.*,S.nama,S.nisn FROM
